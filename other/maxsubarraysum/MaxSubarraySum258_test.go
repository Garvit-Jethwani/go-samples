...
func TestMaxSubarraySum(t *testing.T) {
	var testCases = []struct {
		...
		{[]int{1, -2, 3, 4, -2, 6, -8, 9}, 15},    // Testing Positive Sequence with varying lengths
		{[]int{-1, -2, -3, -4, -5}, -1},           // Testing All Negative Numbers
		{[]int{}, 0},                              // Testing Empty Array
		{[]int{9}, 9},                             // Testing Single Element Array (Positive element)
		{[]int{-9}, -9},                           // Testing Single Element Array (Negative element)
		{[]int{0}, 0},                             // Testing Single Element Array (Zero as element)
		{[]int{0, -9, 10, -1, 0}, 10},             // Testing Array with Zeros
	}

	for _, testCase := range testCases {
		result := MaxSubarraySum(testCase.array)
		if result != testCase.expected {
			t.Errorf("Error: Expected: %v, got: %v", testCase.expected, result)
		} else {
			t.Logf("Correct: Expected: %v, got: %v", testCase.expected, result)
		}
	}
}
