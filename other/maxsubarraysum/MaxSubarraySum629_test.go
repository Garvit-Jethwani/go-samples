...
func TestMaxSubarraySum629(t *testing.T) {
	tests := []struct {
		name           string
		arr            []int
		expectedResult int
	}{
		{"Positive Sequence", []int{2, 3, -7, 6, 5, 4, -1, -2, 1, 6}, 15},
		{"All Negative Numbers", []int{-1, -2, -3, -4, -5}, -1},
		{"Empty Array", []int{}, 0},
		{"Single Element Array", []int{-5}, -5},
		{"Single Element Array", []int{0}, 0},
		{"Single Element Array", []int{5}, 5},
		{"Array with Zeros", []int{0, 0, 0, 0, 0, 0}, 0},
		{"Array with Zeros", []int{0, -2, -3, -4, -5}, 0},
		{"Array with Zeros", []int{-1, -2, 0, -4, -5}, 0},
		{"Array with Zeros", []int{1, 2, 0, 4, 5}, 12},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := MaxSubarraySum(tt.arr)
			if got != tt.expectedResult {
				t.Errorf("got = %v; want %v", got, tt.expectedResult)
			}
		})
	}
}
