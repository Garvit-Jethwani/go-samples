...
// TestMaxSubarraySum868 for testing the MaxSubarraySum function
func TestMaxSubarraySum868(t *testing.T) {
    tests := []struct {
        input []int
        output int
    }{
    ...
    for _, test := range tests {
        result := MaxSubarraySum(test.input)
        if result != test.output {
            t.Errorf("MaxSubarraySum was incorrect, got: %d, want: %d.", result, test.output)
        }
    }
}
